...
def generar_pdf(path, lineas, cliente, descuento_extra=0, descuento_extra_label="",
                comision=0, comision_label="", comision_suma=True, opciones=None, mostrar_dto=True, incluir_catalogo=False):
    """
    ...
    # ── CATÁLOGO ─────────────────────────────────────────────────────────
    if incluir_catalogo:
        catalogo_dir = os.path.dirname(__file__)
        for i in range(1, 9):
            img_path = os.path.join(catalogo_dir, f"{i}.jpeg")
            if os.path.exists(img_path):
                c.showPage()
                # Página completa con la imagen del catálogo
