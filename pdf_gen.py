...
def generar_pdf(path, lineas, cliente, descuento_extra=0, descuento_extra_label="",
                comision=0, comision_label="", comision_suma=True):
    """
    ...
    # Cabecera tabla
    cols = [cw*0.52, cw*0.08, cw*0.20, cw*0.20]
    c.setFillColor(AZUL)
    ...
    for i, ln in enumerate(lineas):
        ...
        c.drawString(ml+3*mm, y-5*mm, str(ln['desc']))
        c.drawCentredString(ml+cols[0]+cols[1]/2, y-5*mm, str(ln['ud']))
        if incluido:
            c.setFillColor(colors.HexColor('#2a6e3f'))
            c.setFont("Helvetica-Oblique", 8)
            c.drawRightString(ml+cols[0]+cols[1]+cols[2]-3*mm, y-5*mm, "Incl. en la puerta")
            c.drawRightString(w-mr-3*mm, y-5*mm, "Incl. en la puerta")
            c.setFillColor(NEGRO); c.setFont("Helvetica", 8.5)
        else:
            c.drawRightString(ml+cols[0]+cols[1]+cols[2]-3*mm, y-5*mm, fmt(ln['precio_u']))
            c.drawRightString(w-mr-3*mm, y-5*mm, fmt(ln['total']))
        y -= rh
    ...
    notas = [
        ...
        "• Descuento 5% PPP aplicado sobre el total.",
