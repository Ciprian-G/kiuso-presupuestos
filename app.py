...
[data-testid="stInfo"] p             { font-size: 15px !important; }
p, div, span                         { font-size: 15px; }

.seccion { background: #fffefb; border-radius: 14px; padding: 24px 28px; margin-bottom: 16px; border: 1.5px solid #e5e0d8; box-shadow: 0 2px 8px rgba(26,20,10,0.04); }
.sec-titulo { font-size: 11px !important; font-weight: 700 !important; letter-spacing: 3px; text-transform: uppercase; color: #1a2a4a; border-bottom: 2px solid #1a2a4a; padding-bottom: 10px; margin-bottom: 18px; }
.cat-header { font-size: 10px !important; font-weight: 700 !important; letter-spacing: 2.5px; text-transform: uppercase; color: #8a7f70; margin: 16px 0 6px 0; padding-bottom: 5px; border-bottom: 1px solid #e5e0d8; }

.resumen-card { background: #1a2a4a; color: #f0ece4; border-radius: 14px; padding: 26px; }
.resumen-titulo { font-family: 'Cormorant Garamond', serif; font-size: 24px; font-weight: 600; color: white; margin: 0 0 18px 0; border-bottom: 1px solid rgba(255,255,255,0.15); padding-bottom: 14px; }
...
.total-box { background:white; color:#1a2a4a; border-radius:10px; padding:16px 20px; margin-top:18px; display:flex; justify-content:space-between; align-items:center; }
.total-lbl { font-size:11px !important; font-weight:700 !important; letter-spacing:2px; text-transform:uppercase; color:#888; }
.total-imp { font-family:'Cormorant Garamond',serif; font-size:32px; font-weight:600; color:#1a2a4a; }
...
with ct:
    st.markdown("""<div style="padding-top:8px">
        <div style="font-family:'Cormorant Garamond',serif;font-size:32px;color:#1a2a4a;font-weight:600;letter-spacing:-0.5px">Generador de Presupuestos</div>
...
with col_form:
    ...
    # EXTRAS — índice global para evitar keys duplicados
    st.markdown('<div class="seccion"><div class="sec-titulo">Extras y accesorios</div>', unsafe_allow_html=True)
    extras_sel = []
    CATS = [
        ("PUERTA",              ["Extra Grado 4","Extra aislante lana de roca","Extra apertura invertida","Extra pintado / barnizado RAL","Extra Revólver antibala FB2","Extra Magnum antibala FB3","Incremento negro  (solo KXXI)","Express especial","Express muy especial","Precerco soldadura","Precerco Grado 5"]),
        ("CERRAJERÍA",          ["Cerradura eléctrica CISA","Cerradero eléctrico","Cerradura antipánico  (sin cilindro)","Cerradura emergencia + barra toallero","Muelle cierrapuertas RC3/RC4","Muelle cierrapuertas RC5/RF","Extra 3D Key","Extra 3D Key doble  (sin cilindro)","Electropistón","Contacto magnético","Protector imán"]),
        ("CAMBIOS CERRADURA",   ["Cambio cerradura K-XXI","Cambio cerradura Torex/Reina","Cambio cerradura borjas a Matrix/Abus","Cambio cerradura borjas a Expert","Cambio cerradura borjas a AP4S","Cambio cerradura Expert SAT","Cambio cerradura Expert Extrem","Cambio cerradura Torex/Reina Matrix","Cambio cerradura Torex/Reina Expert","Cambio cerradura MIA","Cambio doble — cilindros Matrix","Cambio doble — cilindros Expert","Cambio doble — cilindros ISEO","Bloque MIA con 5 llaves"]),
        ("LLAVES",              ["Llave extra Kaba Matrix","Llave extra Kaba Expert","Llave extra AP3S/AP4S","Llave extra Iseo R7","Llave extra Abus Magnet","Llave extra Keso","Llave extra borjas","Llave extra protector imantado","Porte envío de llaves"]),
        ("DOMÓTICA",            ["Tedee","Adaptador Tedee","Teclado Tedee","Bridge Tedee","Danalock con pilas","Teclado Danalock","X1R EVO — Lector huella  (teclado digital)","X1R EVO — Gateway  (pasarela internet)","X1R EVO — Barra toallero","X1R EVO — Precio total con lector huella"]),
        ("TABLERO / CRISTAL",   ["Extra tablero H","Cristal corto alojado en tablero","Cristal largo alojado en tablero","Incrustaciones en INOX","Extra decoración según croquis","Manipulación tablero del cliente","Tachuelas  (por unidad)"]),
        ("REMATES Y ACABADOS",  ["Escalón drenante","Escalón nuevo de agua","Cortavientos  (sin escalón)","Forro de cerco","Zócalo hasta 30 cm","Vierteaguas","Vierteaguas con burlete","F1 burlete agua  (juego)","F1 + F1 agua + 29 + 68  (con burlete)","Media caña  (juego)","Tratamiento intemperie","Pintura cerco RAL standard","Pintura cerco RAL especial","Lacar perfilería aluminio RAL","Precio burlete  (por metro)"]),
        ("HERRAJES",            ["Protector THS","Protector polvo diamante","Mirilla cromo / dorado","Mirilla digital","Mirilla digital grabadora","Tirador 600 mm cromo","Tirador 1000 mm cromo","Tirador 600 mm negro","Tirador 1000 mm negro","Doble manilla","Pomo cromo o dorado","Manilla roseta","Manilla larga borjas","Juego capuchones"]),
        ("CRISTAL EN HOJA",     ["Cristal largo en hoja","Cristal largo en hoja con reja","Cristal corto en hoja","Cristal corto en hoja con reja"]),
        ("INSTALACIÓN",         ["Mano de obra  (por hora)","Instalación albañilería","Instalación soldadura","Desplazamiento  (incluye 30 min)","Desmontaje puerta acorazada","Panel provisional","Recuperar reja  (por reja)"]),
    ]
    # Mapa nombre→precio para lookup rápido
    extras_map = {e[0]: e[1] for e in EXTRAS}

    exc = st.columns(2)
    global_ex_idx = 0
    for cat_name, nombres in CATS:
        items = [(n, extras_map[n]) for n in nombres if n in extras_map]
        if not items: continue
        st.markdown(f'<div class="cat-header">{cat_name}</div>', unsafe_allow_html=True)
        for nom, pre in items:
            col = exc[global_ex_idx % 2]
            checked = col.checkbox(f"{nom}  ({pre} €)", key=f"ex_{global_ex_idx}")
            if checked:
                ud = 1
                if any(x in nom for x in ["THS","Tirador","Mirilla","Tedee","hora","metro","unidad","reja","capuchones","Llave","burlete"]):
                    ud = col.number_input("Ud.", min_value=1, max_value=100, value=1, key=f"exud_{global_ex_idx}")
...
lineas = []
lineas.append({"desc": f"{modelo_sel} · {escala_sel}", "ud":1, "precio_u":precio_puerta, "total":precio_puerta})

ti_p = precio_tab(ti_data, ti_manual, ti_mol_precio)
lineas.append({"desc": f"Tablero interior — {desc_tab(ti_sel,ti_data,ti_mol_precio)}", "ud":1, "precio_u":ti_p, "total":ti_p})

te_p = precio_tab(te_data, te_manual, te_mol_precio)
lineas.append({"desc": f"Tablero exterior — {desc_tab(te_sel,te_data,te_mol_precio)}", "ud":1, "precio_u":te_p, "total":te_p})
...
if doble:
    ...
    lineas.append({"desc":cil_sel if not es_trastero else "Cilindro Iseo R7", "ud":1, "precio_u":cil_precio, "total":cil_precio})
...
with col_prev:
    ...
    if dto_pct > 0:
        ...
        st.markdown(f'<div class="dto"><span>{lbl}</span><span>− {fmt(dto_imp)}</span></div>', unsafe_allow_html=True)
    if com_pct > 0:
        lbl  = com_lbl or f"Comisión {com_pct}%"
        sign = "+" if com_op == "Se suma" else "−"
        col_com = "#7ec8a0" if com_op == "Se suma" else "#f08878"
        st.markdown(f'<div class="dto" style="color:{col_com}"><span>{lbl}</span><span>{sign} {fmt(com_imp)}</span></div>', unsafe_allow_html=True)

    st.markdown(f"""</div>
    <div class="total-box">
        <div class="total-lbl">Total IVA incl.</div>
