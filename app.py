...
with col_form:
    ...
    with tic:
        ...
        ti_incluido = st.checkbox("Incluido en la puerta", key="ti_inc",
    ...
    with tec:
        ...
        te_incluido = st.checkbox("Incluido en la puerta", key="te_inc",
...
if ti_base is not None:
    lineas.append({"desc": f"Tablero interior — {desc_tab(ti_sel,ti_data,ti_desc_custom)}",
                   "ud":1, "precio_u":0 if ti_incluido else ti_base,
                   "total":0 if ti_incluido else ti_base, "incluido": ti_incluido})
    if ti_mol_precio > 0:
        ti_mol_nom = ti_mol.split("  ")[0]
        lineas.append({"desc": f"Moldurado interior — {ti_mol_nom}",
                       "ud":1, "precio_u":ti_mol_precio, "total":ti_mol_precio})

te_base = precio_tab(te_data, te_manual, 0)
if te_base is not None:
    lineas.append({"desc": f"Tablero exterior — {desc_tab(te_sel,te_data,te_desc_custom)}",
                   "ud":1, "precio_u":0 if te_incluido else te_base,
                   "total":0 if te_incluido else te_base, "incluido": te_incluido})
    if te_mol_precio > 0:
        te_mol_nom = te_mol.split("  ")[0]
        lineas.append({"desc": f"Moldurado exterior — {te_mol_nom}",
                       "ud":1, "precio_u":te_mol_precio, "total":te_mol_precio})
