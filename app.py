...
with col_form:
    ...
    sin_puerta = st.checkbox("Sin puerta  (solo recambios / extras)", key="sin_puerta", value=False)
    if sin_puerta:
        modelo_sel = ""; escala_sel = ""; precio_puerta = 0; es_trastero = False
        st.caption("Presupuesto sin puerta — añade los conceptos que necesites en las secciones de abajo.")
    else:
    ...
    if not es_trastero:
        if doble:
            ...
            # Sin puerta → Sin cilindro por defecto (índice 0); con puerta → Kaba Expert (índice 1)
            cil_default = 0 if sin_puerta else 1
            cil_sel    = st.selectbox("Cilindro", [c[0] for c in CILINDROS], index=cil_default)
    ...
    if not es_trastero:
        ...
        n_jambas = jc1.number_input("Juegos de jambas", min_value=0, max_value=4,
                                     value=0 if sin_puerta else (1 if es_aluminio else 2), step=1)
        emb_opts = ["Sin embocadura","≤150 mm  (98 €)",">150 mm  (112 €)",
                    "MDA ≤150 mm  (158 €)","MDA >150 mm  (197 €)",
                    "Aluminio ≤150 mm  (181 €)","Aluminio >150 mm  (217 €)"]
        emb_sel  = jc2.selectbox("Embocadura", emb_opts,
                                  index=0 if (sin_puerta or es_aluminio) else 1)
        if es_aluminio:
        ...
        f1_check = f1_col1.checkbox("F1+29+68  (134 €/ud — remate tablero aluminio)", key="f1_check",
                                     value=(es_aluminio and not sin_puerta))
...
# 1. Puerta (solo si no es presupuesto sin puerta)
if not sin_puerta:
...
with col_prev:
    ...
    # ── CATÁLOGO ──────────────────────────────────────────────────────────
    incluir_catalogo = st.checkbox("Incluir catálogo K100/KXXI al final del PDF", value=True, key="catalogo")
