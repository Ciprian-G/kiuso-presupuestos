...
lineas.append({"desc": f"Tablero interior — {desc_tab(ti_sel,ti_data,0)}",
               "ud":1, "precio_u":ti_base, "total":ti_base})
if ti_mol_precio > 0:
    ti_mol_nom = next(m[0].split("  ")[0] for m in MOLDURADOS if m[1] == ti_mol_precio)
...
lineas.append({"desc": f"Tablero exterior — {desc_tab(te_sel,te_data,0)}",
               "ud":1, "precio_u":te_base, "total":te_base})
if te_mol_precio > 0:
    te_mol_nom = next(m[0].split("  ")[0] for m in MOLDURADOS if m[1] == te_mol_precio)
